...
if ($kategori) {
	$produk = tampil("SELECT * FROM produk WHERE kategoriID = '$kategori'");
	$kategoriname = tampil("SELECT * FROM kategori WHERE kategoriID = $kategori");
}else{
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="vendor/css/stylep.css">
...
<div class="content">
	<div class="container-fluid">
<?php foreach ($kategoriname as $data){ ?>
<h1>hijab <?= $data['namakategori']?></h1>
<?php } ?>
	<?php 
	$numcol = 4;
	$countrow = 0;
	...
	<?php foreach ($produk as $data) { ?>
		...
		<div class="col-lg-<?= $colwidth; ?> mt-5">
	<a href="detailproduk.php?Produk=<?= $data['produkID']; ?>">
				<img class="rounded" src="admin/uploaded_files/<?= $data['GambarProduk'] ?>" height="250" width="200">
			<div class="info-produk">
				<span><p><?= $data['namaproduk'] ?></p></span>
			</div>
			</a>
			<div>
				<span><p>Rp. <?= number_format($data['harga'],2,',','.') ?></p></span>
			</div>
		</div>	
	<?php 
	$countrow++;
